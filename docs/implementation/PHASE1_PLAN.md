...
| 02 Premise 完整闭环 | Task/Candidate/PendingDecision/DecisionRef/ArtifactRevision/Checkpoint；`task open / candidate submit / validate / present / decide / checkpoint` 六动词；premise output contract；Extension 六工具（decide UI 中介）；确定性 L2 | 2A 本分支完成（待 2B）——两段堆叠 PR：`goal/02a-premise-core`（Core）→ `goal/02b-premise-pi`（Pi 交互） |
